...
parser.add_argument('--task_name', type=str, default='mito')
parser.add_argument('--model_type', type=str, default='vit_b_ori')
parser.add_argument('--checkpoint', type=str, default='path to ckpt')
...
class BaseTrainer:
    ...
    def set_loss_fn(self):
        ...
        self.lsd_loss = MSELoss(reduction='mean')
    ...
    def set_optimizer(self):
        ...
        self.optimizer = torch.optim.AdamW([
            ...
            {'params': sam_model.lsd_encoder.parameters() , 'lr': self.args.lr * 0.1},
    ...
    def batch_forward(self, sam_model, image_embedding, gt3D, lsd_embedding):
        ...
        low_res_masks, iou_predictions = sam_model.mask_decoder(
            ...
            lsd3D = lsd_embedding, # (10,256,64,64)
            multimask_output=False,
        )
        prev_masks = F.interpolate(low_res_masks, size=gt3D.shape[-3:], mode='trilinear', align_corners=False)
        return low_res_masks, prev_masks, lsd_embedding


    def interaction(self, sam_model, image_embedding, lsd_embedding, gt3D, gtlsd):
        ...
        # without point
        low_res_masks, prev_masks, prev_lsd = self.batch_forward(sam_model, image_embedding, gt3D, lsd_embedding)
        return_loss = self.seg_loss(prev_masks, gt3D)
        lsd_loss = self.lsd_loss(prev_lsd, gtlsd)
        return_loss += lsd_loss
    ...
    def train_epoch(self, epoch):
        ...
        step_loss = 0
        for step, (image3D, gt3D, gtlsd) in enumerate(tbar):
            ...
            with my_context():
                ...
                gtlsd = gtlsd.to(device).type(torch.long)
                with amp.autocast():
                    image_embedding = sam_model.image_mamba_encoder(image3D)
                    lsd_embedding = sam_model.lsd_encoder(image3D)

                    pred_list = []

                    prev_masks, loss = self.interaction(sam_model, image_embedding, lsd_embedding, gt3D, gtlsd)
