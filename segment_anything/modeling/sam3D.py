...
from .lsd_encoder import Convolve
from .image_mamba_encoder import Fact_mamba_ImageEncoderViT
...
class Sam3D(nn.Module):
    ...
    def __init__(
        self,
        image_encoder: Fact_mamba_ImageEncoderViT,
        lsd_encoder: Convolve,
        ...
        Arguments:
          image_encoder (Fact_mamba_ImageEncoderViT): The backbone used to encode the
            image into image embeddings that allow for efficient mask prediction.
          lsd_encode (Convolve): encodes lsd embeddings.
    ...
    def forward(
        ...
        for image_record, curr_embedding in zip(batched_input, image_embeddings):
            lsd_embeddings = self.lsd_encoder(batched_input)
            low_res_masks, iou_predictions = self.mask_decoder(
                image_embeddings=curr_embedding.unsqueeze(0),
                image_pe=self.prompt_encoder.get_dense_pe(),
                lsd_embeddings = lsd_embeddings.unsqueeze(0),
