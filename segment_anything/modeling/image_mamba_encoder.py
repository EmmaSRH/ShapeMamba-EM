...
class Fact_mamba_ImageEncoderViT(nn.Module):
    def __init__(
            self,
            depth: int = 12,
            embed_dim: int = 768,
        ...
        assert r > 0
        self.sam_model = SamMed3D
        self.sam_model.image_encoder = self.sam_model.ImageEncoderViT3D(
            depth=depth,
            embed_dim=depth,
            img_size=img_size,
            mlp_ratio=mlp_ratio,
        ...
        base_vit_dim = out_chans
        # dim = base_vit_dim
        if fact_layer:
            self.fact_layer = fact_layer
        else:
            self.fact_layer = list(
                range(len(self.sam_model.image_encoder.blocks)))
        ...
        # let's freeze pre-trained weights
        for k, v in self.sam_model.image_encoder.named_parameters():
            if not '.adapter_' in k:
                v.requires_grad = False

        # add factors
        for t_layer_i, blk in enumerate(self.sam_model.image_encoder.blocks):
            ...
            blk.attn = _Fact_mamba_Attention(blk.attn)

        self.ImageEncoderViT = _Fact_mamba_Block(blk)
...
class _Fact_mamba_Block(nn.Module):
    def __init__(
            self,
            Block: nn.Module,
    ):
        super().__init__()
        self.Block = Block
        self.mamba_model = Mamba(d_model=self.dim, d_state=16, d_conv=4, expand=2)
    ...
    def forward(self, x: torch.Tensor, FacTu, FacTv, d_size) -> torch.Tensor:
        ...
        x_mamba = self.mamba_model(x)
        x = shortcut + x_mamba
        x_ln = self.Block.adapter_norm(x)
        x_mlp = self.mlp(x_ln)
        x = shortcut + x_mlp
        # end 3D mamba adapter
...
class _Fact_mamba_Attention(nn.Module):
    def __init__(
            self,
            Attention: nn.Module,
    ):
...
class Mamba_adapt_Encoder(nn.Module):
    """Applies low-rank adaptation and mamba to a Sam model's image encoder.

    Args:
        sam_model: a vision transformer model, see base_vit.py
        r: rank of FacT_tt
        num_classes: how many classes the model output, default to the vit model
        FacT_mamba_layer: which layer we apply FacT_mamba.

    """

    def __init__(self, sam_model: sam, r: int, fact_layer=None, s=1):  # s scale parameter
        super(Mamba_adapt_Encoder, self).__init__()
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            blk.attn = _Fact_mamba_Attention(blk.attn)
            sam_model.image_encoder.blocks[t_layer_i] = _Fact_mamba_Block(blk)

        return _Fact_tt_ImageEncoderViT(sam_model.image_encoder, self.FacTu, self.FacTv)
    ...
    def save_parameters(self, filename: str) -> None:
        ...
        # save lsd encoder, only `state_dict`, the `named_parameter` is not permitted
        ...
        for key, value in state_dict.items():
            if 'lsd_encoder' in key:
                lsd_encoder_tensors[key] = value
        ...
        merged_dict = {**a_tensors, **b_tensors, **FacTu_tensors, **FacTv_tensors, **lsd_encoder_tensors,
    ...
    def load_parameters(self, filename: str) -> None:
        ...
        # load lsd encoder
        lsd_encoder_keys = [k for k in sam_keys if 'lsd_encoder' in k]
        lsd_encoder_values = [state_dict[k] for k in lsd_encoder_keys]
        lsd_encoder_new_state_dict = {k: v for k, v in zip(lsd_encoder_keys, lsd_encoder_values)}
        sam_dict.update(lsd_encoder_new_state_dict)
