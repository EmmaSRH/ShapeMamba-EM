...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        lsd3D:  torch.Tensor,           #[10, 256, 64, 64]
        ...
        Arguments:
          ...
          lsd3D: ( torch.Tensor):  the embeddings of the lsd
    ...
    def predict_masks(
        ...
        out_lsd3D = output_tokens.unsqueeze(0).expand(output_tokens.size(0), -1, -1)
        # tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)
        tokens = torch.cat((output_tokens, out_lsd3D), dim=1)
