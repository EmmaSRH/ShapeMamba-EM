...
class Dataset_Union_ALL(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        sitk_lsd = sitk.ReadImage(self.lsd_paths[index])
        ...
        subject = tio.Subject(
            ...
            lsd = tio.LabelMap.from_sitk(sitk_lsd),
        )
        ...
        if self.mode == "train" and self.data_type == 'Tr':
            return subject.image.data.clone().detach(), subject.label.data.clone().detach(),subject.lsd.data.clone().detach()
        else:
            return subject.image.data.clone().detach(), subject.label.data.clone().detach(), subject.lsd.data.clone().detach(), self.image_paths[index]
        ...
        for path in paths:
            ...
            if os.path.exists(d):
                for name in os.listdir(d):
                    ...
                    self.lsd_paths.append(label_path.replace('labels', 'lsds'))
...
class Test_Single(Dataset): 
    def __init__(self, paths, image_size=256, transform=None, threshold=500):
        self.paths = paths
    ...
    def _set_file_paths(self, paths):
        ...
        self.lsd_paths.append(paths.replace('images', 'lsds'))



if __name__ == "__main__":
    test_dataset = Dataset_Union_ALL(
        paths=['path/to/em_data/',],
        data_type='Ts', 
